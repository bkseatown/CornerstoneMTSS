/**
 * specialist-hub.js — Context-first Specialist Hub runtime
 *
...
(function teacherHubV2() {
  ...
  /* ── Specialist Hub Storage Module (Phase 1 refactoring) ──── */

  var hubStorage = window.createSpecialistHubStorageModule && typeof window.createSpecialistHubStorageModule === "function"
    ? window.createSpecialistHubStorageModule({ hubMemory: hubMemory, todayIsoKey: todayIsoKey })
    : null;


  // Storage module wrapper functions for backward compatibility
  function blockOpenMemory(blockId) {
    return hubStorage && typeof hubStorage.blockOpenMemory === "function"
      ? hubStorage.blockOpenMemory(blockId)
      : { opens: 0, lastOpenedAt: "" };
  }

  function readRecommendationVerdict(studentId) {
    return hubStorage && typeof hubStorage.readRecommendationVerdict === "function"
      ? hubStorage.readRecommendationVerdict(studentId)
      : "";
  }

  function readRecommendationOutcome(studentId) {
    return hubStorage && typeof hubStorage.readRecommendationOutcome === "function"
      ? hubStorage.readRecommendationOutcome(studentId)
      : "";
  }
  ...
  function buildHubRoute(params) {
    try {
      var next = new URL("specialist-hub.html", window.location.href);
  ...
  var HUB_SEARCH_RESOURCES = [
    ...
    { id: "tool-workspace", kind: "resource", label: "My Workspace", subtitle: "Weekly insights, meetings, history, and exports", href: "my-workspace.html" }
  ];
  ...
  function renderQuickAccessRail(contextData) {
    return [
      ...
      '    <a class="th2-command-action" href="my-workspace.html"><strong>Open My Workspace</strong><span>Open reports, progress insights, and meeting-ready history.</span></a>',
      "  </div>",
      "</section>"
    ].join("");
  }
  ...
  function toActivityHref(launch, studentId) {
    ...
    else if (module === "SentenceStudio")   { base = "sentence-studio.html"; }
    else if (module.indexOf("Numeracy") === 0) { base = "number-lab.html"; }
  ...
  var uiModule = window.createSpecialistHubUIModule && typeof window.createSpecialistHubUIModule === "function"
    ? window.createSpecialistHubUIModule({ escapeHtml: escapeHtml })
    : null;
  ...
  function openDrawer(studentId) {
    ...
    if (drawerBody) {
      drawerBody.innerHTML = [
        ...
        '  </div>',
        '  <div class="th2-drawer-row">',
        '    <a class="th2-drawer-link" href="my-workspace.html?student=' + encodeURIComponent(studentId) + '&tab=meeting&from=hub">Open My Workspace &rarr;</a>',
        '  </div>',
        '</div>'
      ].join("\n");
      ...
      if (quickLogBtn) {
        quickLogBtn.addEventListener("click", function () {
          ...
          if (Evidence && typeof Evidence.appendSession === "function") {
            safe(function () {
              Evidence.appendSession(studentId, "reading-lab", {
      ...
      if (copySumBtn) {
        copySumBtn.addEventListener("click", function () {
          ...
          var actHref = window.location.origin + "/" + "specialist-hub.html?student=" + encodeURIComponent(studentId);
  ...
  function ensureDemoCaseload() {
    ...
    if (!hubMemory.getString(SEED_KEY, "") && Evidence && typeof Evidence.appendSession === "function") {
      ...
      safe(function () {
        Evidence.appendSession("demo-ava", "reading-lab", { accuracy: 0.72, wpmProxy: 54, selfCorrects: 2 }, now - 12 * DAY);
        Evidence.appendSession("demo-ava", "reading-lab", { accuracy: 0.75, wpmProxy: 58, selfCorrects: 3 }, now - 8  * DAY);
        Evidence.appendSession("demo-ava", "reading-lab", { accuracy: 0.78, wpmProxy: 61, selfCorrects: 2 }, now - 4  * DAY);
        Evidence.appendSession("demo-ava", "reading-lab", { accuracy: 0.81, wpmProxy: 64, selfCorrects: 4 }, now - 1  * DAY);
      });
      /* Liam T. — word quest phonics, struggling */
      safe(function () {
        Evidence.appendSession("demo-liam", "word-quest", { misplaceRate: 0.38, absentRate: 0.12, vowelSwapCount: 4 }, now - 14 * DAY);
        Evidence.appendSession("demo-liam", "word-quest", { misplaceRate: 0.35, absentRate: 0.10, vowelSwapCount: 3 }, now - 9  * DAY);
        Evidence.appendSession("demo-liam", "word-quest", { misplaceRate: 0.40, absentRate: 0.14, vowelSwapCount: 5 }, now - 4  * DAY);
        Evidence.appendSession("demo-liam", "word-quest", { misplaceRate: 0.37, absentRate: 0.11, vowelSwapCount: 4 }, now - 2  * DAY);
      });
      /* Maya R. — reading lab, strong ORF */
      safe(function () {
        Evidence.appendSession("demo-maya", "reading-lab", { accuracy: 0.88, wpmProxy: 87, selfCorrects: 1 }, now - 10 * DAY);
        Evidence.appendSession("demo-maya", "reading-lab", { accuracy: 0.90, wpmProxy: 91, selfCorrects: 2 }, now - 6  * DAY);
        Evidence.appendSession("demo-maya", "reading-lab", { accuracy: 0.92, wpmProxy: 94, selfCorrects: 1 }, now - 3  * DAY);
        Evidence.appendSession("demo-maya", "reading-lab", { accuracy: 0.93, wpmProxy: 97, selfCorrects: 2 }, now - 1  * DAY);
      });
      /* Noah K. — numeracy, moderate */
      safe(function () {
        Evidence.appendSession("demo-noah", "number-lab", { accuracy: 0.68, speedProxy: 42, hints: 3 }, now - 11 * DAY);
        Evidence.appendSession("demo-noah", "number-lab", { accuracy: 0.71, speedProxy: 45, hints: 2 }, now - 7  * DAY);
        Evidence.appendSession("demo-noah", "number-lab", { accuracy: 0.69, speedProxy: 44, hints: 3 }, now - 3  * DAY);
        Evidence.appendSession("demo-noah", "number-lab", { accuracy: 0.73, speedProxy: 48, hints: 2 }, now - 1  * DAY);
      });
      /* Zoe W. — word quest phonics, early stage */
      safe(function () {
        Evidence.appendSession("demo-zoe", "word-quest", { misplaceRate: 0.28, absentRate: 0.08, vowelSwapCount: 2 }, now - 9  * DAY);
        Evidence.appendSession("demo-zoe", "word-quest", { misplaceRate: 0.25, absentRate: 0.07, vowelSwapCount: 1 }, now - 5  * DAY);
        Evidence.appendSession("demo-zoe", "word-quest", { misplaceRate: 0.22, absentRate: 0.06, vowelSwapCount: 1 }, now - 2  * DAY);
      });
  ...
  function renderFocusCard(state) {
    ...
    var reportsHref = "my-workspace.html?student=" + encodeURIComponent(studentId) + "&from=hub";
    ...
    if (summary && summary.lastSession) {
      var moduleNameMap = { "word-quest": "Word Quest", "reading-lab": "Reading Lab",
        "sentence-studio": "Sentence Studio", "writing-studio": "Writing Studio",
        "number-lab": "Number Lab", "quick-log": "Quick Note" };
  ...
  function renderToolsPanel() {
    ...
    el.toolsBody.innerHTML = [
      ...
      '      <a class="th2-tools-action" href="my-workspace.html"><strong>My Workspace</strong><span>Progress notes and meeting-ready history</span></a>',
