...
window.runWordQuestMain = async function runWordQuestMain(context = {}) {
  ...
  const gameplayStats = typeof gameplayStatsModuleFactory === 'function'
    ? gameplayStatsModuleFactory({
        ...
        clock: () => Date.now(),
        emitTelemetry,
        getMidgameBoostKey: () => MIDGAME_BOOST_KEY,
        getMidgameBoostPool: () => MIDGAME_BOOST_POOL,
  ...
  function showInformantHintToast() {
    ...
    gameplayStats?.setHintRequested?.(true);
  ...
  function showStarterWordCard(options = {}) {
    ...
    gameplayStats?.setStarterWordsShown?.(true);
    syncRoundTrackingLocals();
...
// Global outside-click handlers for flyouts/toasts
  document.addEventListener('pointerdown', e => {
    ...
    revealEffects?.maybeDismissDupeToast?.(e.target);
  ...
  function syncRoundTrackingLocals() {
    ...
    currentRoundErrorCounts = gameplaySupport?.normalizeCounterMap?.(roundState.currentRoundErrorCounts) || Object.create(null);
  ...
  function resetRoundTracking(nextResult = null) {
    if (nextResult?.word) {
      gameplayStats?.beginRound?.(nextResult);
    } else {
      gameplayStats?.resetRoundTracking?.();
    }
    syncRoundTrackingLocals();
  }
  ...
  function getMasteryRowsForDisplay() {
    return Object.entries(sessionSummary.masteryBySkill || {})
      .map(([skillKey, row]) => {
        ...
        const errorCounts = gameplaySupport?.normalizeCounterMap?.(row.errorCounts) || Object.create(null);
        const topErrorKey = gameplaySupport?.getTopErrorKey?.(errorCounts) || '';
        return {
          skillKey,
          ...
          avgTimeMs,
          avgTimeLabel: attempts ? (gameplaySupport?.formatDurationLabel?.(avgTimeMs) || '0s') : '--',
          avgTimeSeconds: attempts ? Number((avgTimeMs / 1000).toFixed(1)) : 0,
          topErrorKey,
          topErrorLabel: gameplaySupport?.getTopErrorLabel?.(errorCounts) || '--',
          errorCounts
        };
      })
      .filter(Boolean);
  ...
  function buildMtssIepNoteText() {
    const student = getActiveStudentLabel();
    const now = new Date();
    const topErrorLabel = gameplaySupport?.getTopErrorLabel?.(sessionSummary.errorTotals) || '--';
  ...
  function getMiniLessonKeyFromSession() {
    return gameplaySupport?.resolveMiniLessonErrorKey?.(
      activeMiniLessonKey,
  ...
  function renderMiniLessonPanel() {
    ...
    if (!textEl) return;
    const planText = gameplaySupport?.buildMiniLessonPlanText?.(key, {
      validPlans: ERROR_MINI_LESSON_PLANS
    }) || '';
  ...
  function buildSessionSummaryText() {
    ...
    return [
      ...
      `Avg Time: ${avgTime}`,
      `Top Error Pattern: ${gameplaySupport?.getTopErrorLabel?.(sessionSummary.errorTotals) || '--'}`,
      `Next Instructional Step: ${gameplaySupport?.getInstructionalNextStep?.(sessionSummary.errorTotals) || 'Continue current lesson target.'}`,
  ...
  function buildSessionSummaryCsvText() {
    ...
    const lines = [[
      ...
      gameplaySupport?.getTopErrorLabel?.(sessionSummary.errorTotals) || '--',
      gameplaySupport?.getInstructionalNextStep?.(sessionSummary.errorTotals) || 'Continue current lesson target.',
      topSkill?.label || '--',
  ...
  function buildMissionSummaryText() {
    ...
    if (recent) {
      lines.push(
        ...
        `Level: ${deepDiveBuilders?.getChallengeLevelDisplay?.(recent.level) || recent.level} · Score: ${recent.score}/100 (${recent.scoreBand}) · On time: ${recent.onTime ? 'yes' : 'no'}`
      );
    }
    return lines.join('\n');
  }
  ...
  async function copyMiniLessonPlan() {
    const key = getMiniLessonKeyFromSession();
    await copyTextToClipboard(
      gameplaySupport?.buildMiniLessonPlanText?.(key, {
        validPlans: ERROR_MINI_LESSON_PLANS
      }) || '',
  ...
  _el('mission-lab-level-select')?.addEventListener('change', () => {
    ...
    const level = deepDiveBuilders?.normalizeThinkingLevel?.(_el('mission-lab-level-select')?.value || '', '') || '';
    if (!note || !level) return;
    note.textContent = `Selected thinking target: ${deepDiveBuilders?.getChallengeLevelDisplay?.(level) || level}. Start Deep Dive when ready.`;
  });
  ...
  function syncRevealMeaningHighlight(nextEntry) {
    ...
    const meaning = revealText?.getRevealMeaningPayload?.(nextEntry, shouldIncludeFunInMeaning()) || {
      definition: '',
      funAddOn: '',
      line: '',
      readAll: ''
  ...
  function syncRevealReadCue(nextEntry) {
    ...
    const cue = revealText?.buildRevealReadCue?.(sourceText) || '';
  ...
  const WQMusic = createMusicModule(DEFAULT_PREFS);
// ─── 12. Start ───────────────────────────────────────
