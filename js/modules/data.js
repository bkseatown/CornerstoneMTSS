/**
 * js/modules/data.js — ES module version of WQData
 *
 * Canonical word data loader. Identical logic to js/data.js (IIFE version)
 * but exported as a proper ES module so Vite can bundle and tree-shake it.
 *
 * The original js/data.js remains loaded as a classic <script> for backward
 * compatibility with app.js and all legacy IIFE code during the transition.
 */
...
function _finalize() {
  ...
  if (_isDevMode()) {
    window.WQ_QUARANTINE = { count: quarantined, items: _quarantineItems.slice() };
  }
}
...
async function load() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (raw && typeof raw === 'object') window.WQ_WORD_DATA = raw;
...
function getAllEntries() { return Object.assign({}, _entries); }
function getAllRawEntries() {
  return window.WQ_WORD_DATA && typeof window.WQ_WORD_DATA === 'object'
    ? window.WQ_WORD_DATA : {};
}
function getQuarantineReport() {
  return { count: _quarantineItems.length, items: _quarantineItems.slice() };
}

export default {
  load, getEntry, getPlayableWords, isLoaded,
  getQuarantineReport, hasWord, getAllEntries, getAllRawEntries,
};
