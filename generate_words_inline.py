#!/usr/bin/env python3
"""
generate_words_inline.py — Word Quest
=======================================
Converts app_ready_database_FINAL.json (or the _with_audio version)
into the canonical data/words.json source used by js/data.js.
...
Run from anywhere:
  python3 generate_words_inline.py \
    ...
    --out  /Users/robertwilliamknaus/Desktop/WordQuest/data/words.json
...
def normalize_entry(word, v):
    ...
    # Old WORD_ENTRIES format — convert to new shape
    en = v.get("en") or {}
    return (w, {
        "display_word": w,
...
def main():
    ...
    ap.add_argument("--out",  required=True, help="Output path for canonical words JSON")
    args = ap.parse_args()
    ...
    print(f"✅ {len(normalized)} entries ({playable} playable, {skipped} skipped)")
    ...
    print(f"\n✅ Done! The canonical word data is ready for the app loader.")
